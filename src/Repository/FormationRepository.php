...
class FormationRepository extends ServiceEntityRepository
{
    ...
    /**
     * Récupère les formations pour le back-office
     * avec tri et filtres.
     */
    public function findForBackOffice(
        string $tri = 'date',
        string $ordre = 'DESC',
        ?string $filtreTitre = null,
        ?string $filtrePlaylist = null,
        ...
        if ($filtreTitre) {
            $qb->andWhere('LOWER(f.title) LIKE :titre')
               ->setParameter('titre', '%' . mb_strtolower($filtreTitre) . '%');
        }

        if ($filtrePlaylist) {
            $qb->andWhere('LOWER(p.name) LIKE :playlist')
               ->setParameter('playlist', '%' . mb_strtolower($filtrePlaylist) . '%');
        }
        ...
        switch ($tri) {
            case 'titre':
            case 'title':
                $qb->addOrderBy('f.title', $ordre);
                break;
            case 'playlist':
                $qb->addOrderBy('p.name', $ordre);
                break;
            case 'date':
            default:
                $qb->addOrderBy('f.publishedAt', $ordre);
                break;
        }
